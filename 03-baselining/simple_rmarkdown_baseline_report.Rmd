---
title: "Simple RMarkdown Baseline Report - Mbire"
...
knitr::opts_chunk$set(echo = TRUE, tidy = TRUE, comment = NA, highlight = FALSE, echo = FALSE, message = FALSE, warning = FALSE, fig.width = 8, out.width = '100%', fig.align = 'center', dpi = 150)
...
This document reports the results of a survey of `r nrow(data)` households conducted between the 12th and 17th of December 2022 in Wards 2, 3, 9, 12 and 17 of Mbire District (Fig.1).

# 2. Study area

Figure 1 displays the location of the surveyed farms.

```{r fig1, fig.cap = "Location of the households surveyed in the Wards 2, 3, 9, 12 and 17 of Mbire District"}
...
Table 1 compares mean characteristics of farms in Ward 2, and in Wards 3, 9, 12 and 17. 

Compared to farms/households in Wards 3, 9, 12 and 17, farms/households in Ward 2 are characterized by heads of household statistically younger, and farms smaller with less cattle (Table 1). More farms in Ward 2 have an individual garden, while own production is the main source of food for a smaller percentage of farms. Similarly, farming is the main source of income for a smaller percentage of farms in Ward 2 than in Wards 3, 9, 12 and 17. Households in Ward 2 are food secure for a smaller proportion of the year but their dietary diversity is higher, when compared to households in Wards 3, 9, 12 and 17. Total cereal production and total equipment value are significantly lower for farms in Ward 2 tna farms in Wards 3, 9, 12 and 17.


```{r tab1}

small_dataset$what_is_the_sex_of_the_head_of_the_household = as.numeric(small_dataset$what_is_the_sex_of_the_head_of_the_household == "Female")

small_dataset$what_is_the_education_level_of_the_head_of_the_household = as.numeric(small_dataset$what_is_the_education_level_of_the_head_of_the_household != "Primary level")

small_dataset$main_food_source = as.numeric(small_dataset$main_food_source == "Own production")

small_dataset$main_income_source = as.numeric(small_dataset$main_income_source == "Crop sales" | small_dataset$main_income_source == "Livestock sales")

small_dataset[c(23:36)][is.na(small_dataset[c(23:36)])] = 0
...
Maize area doesn't increase with increasing total cropped area, but the areas in small grains and in other crops (other than maize, small grains and legumes) increase significantly (Figure 2).

```{r fig2, fig.cap = "Land allocation"}
...
Pumpkin is the main species intercropped, followed by cowpea, watermelon and local cucumber (Figure 3).

```{r fig3, fig.cap = "Major intercrop species"}
...
## 4.3. Maize residue use

The large majority of maize residue is either grazed in the field or harvested for feed (Figure 4).

```{r fig4, fig.cap = "Maize residue use"}
...
Food security is the lowest between October and January (Figure 5).

```{r fig5, fig.cap = "Food security situation in the 12 months preceding the interview"}
...
The majority of farmers in Mbire perceive soil fertility to be declining and almost half of the population surveyed perceives soil erosion to be worsening. However, the majority of farmers felt tree cover and sil compaction were either stable or improving.

```{r fig6, fig.cap = "Perception of Change in soil health"}
